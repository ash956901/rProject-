...
textbook_pdf <- "/home/ashutosh/Documents/rProject/hh.pdf"
...
ggplot(curriculum_tfidf_df[1:top_n, ], aes(x = reorder(term, tfidf_curriculum), y = tfidf_curriculum)) +
  ...
  coord_flip() +
  ggtitle("Top 10 TF-IDF Terms in Curriculum") +
  xlab("Term") +
  ylab("TF-IDF")
...
ggplot(textbook_tfidf_df[1:top_n, ], aes(x = reorder(term, tfidf_textbook), y = tfidf_textbook)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  ggtitle("Top 10 TF-IDF Terms in Textbook") +
  xlab("Term") +
  ylab("TF-IDF")
...
curriculum_topics <- posterior(lda_model)$topics[1, ]
textbook_topics <- posterior(lda_model)$topics[2, ]

topic_alignment_score <- sum(pmin(curriculum_topics, textbook_topics)) / sum(curriculum_topics)

# Print topic alignment results
cat("Topic Alignment Score (0 to 1):", topic_alignment_score, "\n")
cat("Curriculum Topics:\n")
print(curriculum_topics)
cat("Textbook Topics:\n")
print(textbook_topics)
